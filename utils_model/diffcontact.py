...
def capsule_sdf_per_joint(object_verts, object_normals, hand_points, hand_normals, part_labels, caps_rad, caps_top, caps_bot, robot_name):
    batch_size = object_verts.shape[0]
    num_obj_verts = object_verts.shape[1]
    num_joints = 16
    
    all_sdf = torch.zeros((batch_size, num_obj_verts, num_joints), device=object_verts.device)
    all_dot = torch.zeros((batch_size, num_obj_verts, num_joints), device=object_verts.device)
    if part_labels.dim() == 1:
        part_labels = part_labels.unsqueeze(0).expand(batch_size, -1)
    
    for joint_idx in range(num_joints):
        if robot_name == 'barrett':
            joint_mask = (part_labels == joint_idx) & (part_labels != 0)
        else:
            joint_mask = (part_labels == joint_idx) # & (part_labels != 0)
        joint_mask = joint_mask.unsqueeze(-1).repeat(1, 1, 3)
        if not torch.any(joint_mask):
            continue

        sdf, dot = capsule_sdf(object_verts, object_normals, 
                                hand_points*joint_mask, hand_normals*joint_mask, 
                                caps_rad, caps_top, caps_bot, True)
        all_sdf[:, :, joint_idx] = sdf
        all_dot[:, :, joint_idx] = dot
        
    return all_sdf, all_dot
