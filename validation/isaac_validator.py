...
class IsaacValidator:
    ...
    def run_sim(self):
        ...
        object_pose_grasped = gymtorch.wrap_tensor(self._rigid_body_states).clone()[::self.rigid_body_num, :7]
        object_transform_grasped = np.eye(4)[np.newaxis].repeat(self.batch_size, axis=0)
        object_transform_grasped[:, :3, 3] = object_pose_grasped[:, :3]
        object_transform_grasped[:, :3, :3] = Rotation.from_quat(object_pose_grasped[:, 3:7]).as_matrix()
        dof_states_grasped = gymtorch.wrap_tensor(self._dof_states).clone().reshape(len(self.envs), -1, 2)[:, :, 0]
        robot_transform_grasped = np.eye(4)[np.newaxis].repeat(self.batch_size, axis=0)
        robot_transform_grasped[:, :3, 3] = dof_states_grasped[:, :3]
        robot_transform_grasped[:, :3, :3] = Rotation.from_euler('XYZ', dof_states_grasped[:, 3:6]).as_matrix()
        robot_transform_grasped = np.linalg.inv(object_transform_grasped) @ robot_transform_grasped
        dof_states_grasped[:, :3] = torch.tensor(robot_transform_grasped[:, :3, 3])
        ...
        # apply inverse object transform to robot to get new joint value (force phase后)
        self.gym.refresh_rigid_body_state_tensor(self.sim)
